...
function parseEnvValue(object) {
  if (object.fromEnvVar && object.fromEnvVar != "null") {
    ...
    if (!value) {
      throw new Error(
        `Attempted to load provider value using \`env(${object.fromEnvVar})\` but it was not present. Please ensure that ${import_chalk.default.dim(
          object.fromEnvVar
...
function parseBinaryTargetsEnvValue(object) {
  if (object.fromEnvVar && object.fromEnvVar != "null") {
    ...
    if (!value) {
      throw new Error(
        `Attempted to load binaryTargets value using \`env(${object.fromEnvVar})\` but it was not present. Please ensure that ${import_chalk.default.dim(
          object.fromEnvVar
        )} is present in your Environment Variables`
      );
    }
