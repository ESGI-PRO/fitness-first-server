...
Using the Data Proxy (connection URL starting with protocol ${import_chalk.default.green(
  "prisma://"
)}) is not supported for this CLI command ${import_chalk.default.green(`prisma ${command}`)} yet. ${command === "studio" ? "" : "Please use a direct connection to your database via the datasource 'directUrl' setting."}
...
async function checkUnsupportedDataProxyMessage(command, args, implicitSchema) {
  ...
  for (const [argName, argValue] of argList) {
    ...
    if (argName.includes("schema")) {
      ...
      const url = command === "studio" ? config.datasources[0]?.url : (0, import__.getEffectiveUrl)(config.datasources[0]);
